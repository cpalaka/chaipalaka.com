...
10. As a visitor, I want a persistent frame bar (taskbar) along the top or bottom of the viewport that carries the site name, current-page indicator, section nav, and a settings menu, so that the chrome stays accessible from every route without competing with the physics foreground.
...
- **Physics anchor positions are the *initial* layout.** Each `Card` computes a CSS-grid anchor position; matter.js spawns the body at that anchor. Subtractive fallbacks across all degradation modes work the same way (no physics → just the grid).
...
- **Card pre-sizing via pretext.** Each card's text size is precomputed via the **TextMeasure** primitive (`text/measure.ts`'s `measure(text, font, maxWidth)`, built on `@chenglou/pretext`) before the body is registered with matter.js, so the body has correct dimensions from the start and we never call `getBoundingClientRect()` per card.
...
> **Note (2026-07-01 — DRAFT-010 / [ADR-0010](docs/adr/0010-drift-default-physics.md)).** The gravity model below is now the **dormant** physics mode: **drift** (top-down plane, gentle Brownian wander + damping + prose repel, no gravity, no home anchors) is the site-wide default and every route converts. Gravity code + knobs stay in the engine; no route declares them. The walls persist as drift bounds; "detached piles on the floor" and per-route gravitational identity (incl. the 404 up-gravity joke) retire with the conversion. Authoritative design: `docs/superpowers/specs/2026-07-01-drift-physics-design.md`.
...
> **Note (2026-05-13).** A minimize-card feature (per-card `−` button → chip in a frame-bar strip → restore) was prototyped earlier and removed in [ADR-0002](docs/adr/0002-remove-minimize-card.md). Route-change is now the only dismissal.
...
> **Note (2026-07-01 — DRAFT-010 / [ADR-0010](docs/adr/0010-drift-default-physics.md)).** Under the drift default: rope semantics and layout-derived lengths are unchanged (static-edge parents become radially derived, matching the `len = distance(...)` formula above); buoyancy is dormant-mode-only; `StringLayer` draws straight lines with tension-driven stroke width/opacity (the sag bezier is gated to the dormant gravity mode); release behavior is drift-settle, not a pendulum swing.
...
- **Hybrid mechanism per case.** Not a single transition system:
  - **Canvas → canvas (route-level):** physics-driven via three primitives composed by `TransitionDirector`:
    ...
    - `anchor-slide` (coupled, T3 horizontal in slice 21, T4 vertical in slice 21b) — each card is put into kinematic mode (`setDragging(true)`) and its position is tween'd along `axis × sign` over ~700ms with an ease-out-cubic curve; one PhysicsWorld holds both card sets briefly during the tween, and cards are released back to dynamic mode at completion. *Why kinematic, not dynamic-with-tether-drag:* an earlier dynamic implementation (parent-anchor tween + pull-only rope force) was tried under #86; at slice 21's durations and the rest of the site's tether stiffness it reads as too chaotic — cards lag the rope, swing on the y-axis, and the coordinated motion stops feeling like a single composed move. The kinematic version sacrifices the pendulum nuance for legibility.
  - **Within-page section pagination (canvas):** the vertical presentation of `anchor-slide` (slice 21b). Hash-fragment routing (`#sN`); per-route opt-in via PageDef `sections` field; ceiling collision in sensor mode during T4 so cards pass through the visible top edge. Next/back are `kind: 'nav'` Cards strung with very short floor tethers.
...
- **Card lifecycle is registry-mediated, Director-orchestrated.** The `CardRegistry` is the source of truth for card presence and lifecycle state (`spawning → active → exiting`); route components register/unregister on mount/unmount via the `<Card>` author API. The Director marks cards `exiting` (via `useLayoutEffect`) before React commits the unmount and defers actual `release()` calls until exit motion completes — see invariant I-2 in `CONTEXT.md`. A `<CardLayer>` mounted at app root renders every registered card, so route subtree unmount does not destroy the visual until the Director releases the card.

- **Public interface.** `<TransitionDirector pageDefs={pageDefs} edges={edges}>` mounted in `CanvasLayout` auto-wires on `useLocation`. Within-page section transitions (slice 21b) are triggered by hash-fragment changes via `useHashSection()` — the Director picks them up the same way it picks up route changes.
...
- **`PhysicsWorld`** — register/unregister round-trips; `linkBodies` / `unlinkBodies` round-trips; impulse and velocity setters move the body in the expected direction. **Drift mode (default):** the drift force pass produces refresh-rate-invariant and mass-invariant wander statistics (dt-normalized velocity kicks; frozen-body determinism via injected RNG); prose-repel force is finite and outward everywhere (box corners + inside-the-rect); drift-mode anchor moves translate-pair (preserve wander offset + velocity), never teleport-and-zero; matter sleeping stays off. **GPU snapshot bridge (task-038, shared by 041):** `snapshotCardRects` packs `[cx, cy, halfW, halfH]` (+ optional `[cos θ, sin θ]`) per non-static card zero-alloc, excludes static registrations, clamps to both buffers' capacity, tracks the live body. **Dormant gravity subset:** `setGravityDirection` correctly sets the gravity vector for all four cardinal directions; a body without a tether falls toward gravity and is caught by the floor/ceiling wall.
- **`auraBridge`** (`canvas/aura/`) — the pure slot-packing half of the physics→GPU aura bridge: active rects + rotations land in the leading GPU slots verbatim, every slot at/beyond count parks off-screen with zero size and identity rotation, count clamps to `maxCards`. (The TSL scene itself is verified visually — jsdom can't run WebGPU.)
...
- **`Tether` / `StringLayer`** — tether is slack when card distance < string length and taut when at length; pull-only force applied correctly (no push force when slack); **continuous tension** from `list()` is zero/clamped when slack, monotonic past rest length, and consistent at the `slackFactor` boundary (the exact formula task-039 consumes); static-edge wiring attaches at the edge point nearest the child with radial length (not the edge-bar center); `StringLayer` maps tension to stroke width/opacity. **Dormant gravity subset:** balloon buoyancy force magnitude matches heavy gravity magnitude at steady state; sag bezier control points produce correct sag direction for slack tethers (mode-gated path).
- **`TransitionDirector`** — dispatch resolution order (edge table → PageDef `exit`/`enter` → history-direction default); history-direction classification (forward/back/sibling) from history index; `string-cut-drop` cuts only direct ceiling tethers and balloons rise symmetrically; `pour-in-drop` chain-order stagger; `anchor-slide` kinematic tween produces the expected position curve for both `axis: 'horizontal'` and `axis: 'vertical'`, with cards returned to dynamic mode at completion; reduced-motion path bypasses physics; orphaned-card portal survives route unmount; `shared-element-morph` dispatches `startViewTransition` only when both routes declare matching `<ViewTransition name>` and auto-excludes the morphing pair from T1/T2.
- **`TextMeasure`** (`text/measure.ts`, with **Font** consts in `text/fonts.ts`) — `measure(text, font, maxWidth)` returns dimensions consistent with what the same text would actually take in the DOM (verified once via a snapshot test, then trusted); a drift test asserts the **Font** consts agree with the matching CSS custom properties in `tokens.css`.
