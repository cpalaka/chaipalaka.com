...
- **`PhysicsWorld`** — register/unregister round-trips; `setGravityDirection` correctly sets the gravity vector for all four cardinal directions; a body without a tether falls toward gravity and is caught by the floor/ceiling wall; `linkBodies` / `unlinkBodies` round-trips; impulse and velocity setters move the body in the expected direction.
