...
2. **`parkAt`'s inline radial-wiring duplicate (spec §3.3) converts in S1**, alongside
   ...
   Coverage exists to gate it in S1: `parkAt` is exercised by `PinnedCard.test.tsx` and
   `PhysicsWorld.contentBox.test.ts` [reuse — verified this session].
...
3. Invariance tests green: dt-normalized, mass-invariant, deterministic under injected
   RNG. [spec §1, §3.8]
4. Prose-repel falloff finite + outward everywhere incl. corners and inside-rect (test).
   [spec §1, §3.8]
5. Dormant gravity subset green under `mode:'gravity'` (V1.2). [spec §3.1]
6. Word-anchor proxies register as sensors; no collision impulse from proxy overlap
   (test). [spec §3.1]
7. Drift-mode anchor moves translate-pair (wander offset + velocity preserved), never
   teleport-and-zero (test). [spec §3.1, D4]
8. Radial edge wiring at BOTH call sites (`wireTetherFor` + `parkAt`): attach point =
   nearest edge point, radial length (tests). [spec §3.3]
9. Atelier layout write-back round-trips gravity-less layouts (V1.4). [spec §3.2]
10. Full verify gate + grep sweep clean (V1.1, V1.6).
...
(phase string), `ambientPins.test.ts`; deletes `spawnOffset` test; keeps
...
5. Parked + word-anchored poses produced by rope + repel jointly (V2.3 smoke evidence).
   [spec §1, §3.3]
6. `peekTuning` knobs re-semanticized in place (`fadeMs`, `dismissKick`); dismissal
   knobs live ONLY in `peekTuning`. [spec D7]
7. Full verify gate + grep sweep clean (V2.1, V2.5).
...
**Scope:** spec §3.6 (both entries), §3.7 (all of it), §3.8's `sectionLayout` deletion,
D1's demo deletions (`sandbox/Strings.tsx` + registration + the `App.tsx:241` stale
...
- Modify: `web/src/canvas/StringLayer.tsx` (+ its test), `web/src/physics/Tether.ts`
  ...
  re-scatter label), `web/src/App.tsx` (route dereg + :241 comment),
  `web/src/__tests__/FrameBar.test.tsx` (:182 name)
- Delete: `web/src/sandbox/Strings.tsx`, `web/src/layout/sectionLayout.ts` + its test
...
`StringLayer.test.tsx` mode-gated sag/straight+styling; `vite-plugin-atelier.test.ts`
fossil half (write-back round-trip stays green); `sectionLayout.test.ts` `[delete]`;
`FrameBar.test.tsx:182` rename.

**Named verification checks:**
- V3.1 full verify gate — build's prerender check matters doubly here (SSG route set
  shrinks with `/sandbox/strings` gone).
...
- V3.4 Atelier write-back round-trip test green post-fossil-sweep; `/atelier` physics
  axis smoke: world group presents as dormant-mode, "re-scatter" label.
...
3. `TetherView` exposes continuous `tension` with the exact formula pinned + boundary
   behavior stated + allocation note recorded for 039. [spec §3.6, §3.8]
...
5. `sectionLayout.ts` + test deleted after verifying zero non-test importers at build
   time. [spec §3.8]
6. `sandbox/Strings.tsx` + its `App.tsx` registration deleted; prerender set shrinks
   accordingly; `App.tsx:241` stale comment removed. [spec D1]
...
**Files:**
- Modify: `web/src/routes/Home.tsx` (:20), `web/src/routes/NotFound.tsx` (:10),
  `web/src/routes/Lifelog/Stuff.layout.ts` (:5), `web/src/routes/Flash.tsx` (:114,:124),
  `web/src/routes/FlashDetail.tsx` (:70), `web/src/test/Box.tsx` (:39) + `BoxB.tsx`
  walkthrough copy, `web/src/pin/wobble.ts` retune values, `driftTuning.ts` +
  `peekTuning.ts` + per-route `driftScale` values (tuning only), reduced-motion wiring
  site (per D8)
...
1. Read spec §§3.2 (route drops), D7, D8, §6 items 1–4 before starting — no other
   context.
2. All six route-file `gravity:` declarations dropped; route tests rewritten; zero
   `pageDef.gravity` assertions remain for drift routes. [spec §3.2, §3.8]
...
7. **Feel constants tuned solo in-session (visual/feel AC — never run as a background
   wave).** [spec §6 item 1]
8. Full verify gate + final global grep sweep clean.
...
| D1 `sandbox/Strings.tsx` + `App.tsx` registration + :241 comment | delete | S3 |
...
| `parkAt` is covered by `PinnedCard.test.tsx` + `PhysicsWorld.contentBox.test.ts` (gates adjustment 2 in S1) | [reuse] | verified this session (grep) |
...
None. No design error found in D1–D10 or the §3 change map. The two §7 sequencing
adjustments above are boundary changes to explicitly non-approved input, with stated
justification — not re-litigation.
