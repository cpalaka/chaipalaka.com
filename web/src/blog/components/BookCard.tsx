...
function fetchBooks(): Promise<BooksApiResponse> {
  if (!booksPromise) {
    booksPromise = fetch('/api/books').then(r => r.json() as Promise<BooksApiResponse>)
  }
  return booksPromise
}
...
export function BookCard({ slug }: BookCardProps) {
  ...
  if (loading) return <span className="book-card book-card--placeholder">—</span>
  if (!book) return <span className="book-card book-card--placeholder">—</span>
