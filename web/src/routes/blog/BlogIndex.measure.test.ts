...
import { measureBlogCard, formatPostDate, CARD_GAP, CARD_PADDING, CARD_PADDING_BOTTOM } from './BlogIndex.measure'
import type { PostFrontmatter } from '../../blog/types'

const fixedMeasure = (_text: string, _fontKey: string, _maxWidth: number) => ({
  width: 100,
  height: 20,
})
...
describe('measureBlogCard', () => {
  ...
  test('height = sum of 5 parts (title+desc+date+tags+cta) + 4 gaps + top + bottom padding', () => {
    ...
    expect(height).toBe(expectedContent + CARD_PADDING + CARD_PADDING_BOTTOM)
  })
  ...
  test('respects maxWidth cap — measure is called with the provided maxWidth', () => {
    const calls: number[] = []
    const spy = (_t: string, _f: string, mw: number) => { calls.push(mw); return { width: 50, height: 10 } }
  ...
  test('width covers the widest part when parts differ', () => {
    let call = 0
    // title (card-title) is wide, others are narrow
    const spy = (_t: string, fontKey: string, _mw: number) => {
      call++
      return fontKey === 'card-title' ? { width: 250, height: 40 } : { width: 100, height: 20 }
    }
