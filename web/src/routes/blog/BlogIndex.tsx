...
import { measureBlogCard, formatPostDate, CARD_PADDING } from './BlogIndex.measure'
...
function computeAnchors(): CardAnchor[] {
  ...
  const specs: CardSpec[] = posts.map(post => {
    ...
    return { id: post.slug, text: post.frontmatter.title, fontKey: 'body', width, height }
  })
        ...
        return (
          ...
          >
            ...
            {post.frontmatter.tags.length > 0 && (
              <p className="card-tags">{post.frontmatter.tags.join(' · ')}</p>
            )}
            <Link to={`/blog/${post.slug}`} className="read-post-link">
              Read post →
            </Link>
          </PhysicsCard>
        )
